...
function HabitAccordion({ habit, isCompleted, onToggle }) {
  const [isExpanded, setIsExpanded] = useState(false);
  const config = getCategoryConfig(habit.category);

  return (
    <div
      className={`relative rounded-lg border-l-4 ${config.borderColor} ${config.bgColor} 
...
export default memo(HabitAccordion);
